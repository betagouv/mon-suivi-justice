...
unless defined?(Rails::Console) || Rails.env.test? || Rails.env.development? || File.split($PROGRAM_NAME).last == 'rake'
  ...
  #
  Rufus::Scheduler.singleton.cron '5 0 * * *', lockfile: '.rufus-scheduler.lock' do
    # Every day at 00:05, create all Slot from SlotTypes
    SlotCreationJob.perform_later
  end

  Rufus::Scheduler.singleton.cron '50 0 * * *', lockfile: '.rufus-scheduler.lock' do
    # Every day at 00:50, delete convicts archived 6 months ago
