...
describe DivestmentPolicy do
  ...
  context('per role') do
    ...
    context('user is cpip') do
      let(:user_to) { create(:user, :in_organization, type: :spip, role: :cpip) }

      it { is_expected.to permit_action(:create) }
    end

    context('user is dpip') do
      let(:user_to) { create(:user, :in_organization, type: :spip, role: :dpip) }

      it { is_expected.to permit_action(:create) }
    end

    context('user is overseer') do
      let(:user_to) { create(:user, :in_organization, type: :spip, role: :overseer) }

      it { is_expected.to permit_action(:create) }
    end

    context('user is educator') do
      let(:user_to) { create(:user, :in_organization, type: :spip, role: :educator) }

      it { is_expected.to permit_action(:create) }
    end

    context('user is psychologist') do
      let(:user_to) { create(:user, :in_organization, type: :spip, role: :psychologist) }

      it { is_expected.to permit_action(:create) }
    end

    context('user is secretary_spip') do
      let(:user_to) { create(:user, :in_organization, type: :spip, role: :secretary_spip) }
