namespace :convicts do
  ...
  # with id1 the id of the organization to import into, id2 the organization source
  ...
  task :link_from_other_organization, %i[id1 id2] => [:environment] do |_task, args|
    organization_target = Organization.find(args[:id1])
    organization_source = Organization.find(args[:id2])
